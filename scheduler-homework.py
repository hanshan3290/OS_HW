...
if options.solve == True:
    ...
    if options.policy == 'RM':
        thetime = 0
        print 'Execution trace:'
        #YOUR CODE
        
        for i in range(0, jobnum):
            for j in range(jobnum, i, -1):
                if joblist[j][1] < joblist[j-1][1]:
                    tempint = joblist[j][1]
                    joblist[j][1] = joblist[j-1][1]
                    joblist[j-1][1] = tempint

                    tempdouble = joblist[j][0]
                    joblist[j][0] = joblist[j-1][0]
                    joblist[j-1][0] = tempdouble
        
        currentTime = 0.0
        for tmp in joblist:
            jobnum  = tmp[0]
            runtime = tmp[1]
            afterTime = currentTime + runtime
            print '[ time   %3.2f ] Run job %3d for %3.2f secs ( DONE at %3.2f )' % (currentTime, jobnum, runtime, afterTime)
    ...
    if options.policy != 'FIFO' and options.policy != 'SJF' and options.policy != 'RR' and options.policy != 'RM': 
